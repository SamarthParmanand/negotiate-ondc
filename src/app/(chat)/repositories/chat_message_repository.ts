...
export default class MessageRepository implements ChatMesssageRepository{
    ...
    constructor(sessionId:string,callback: (msgs: ChatMessage[]) => void){
        ...
        this.channel = this.supabase.channel(`message-${this.sessionId}`)   .on(
            'postgres_changes',
            { event: '*', schema: 'public', table: 'chat_sessions', filter: `sessionId=eq.${this.sessionId}` },
            (payload) => {
                console.log(payload);
            // callback(payload.);
            }
   ...
   async fetchMessages(_offset?: number|30): Promise<ChatMessage[]> {
     const {data,error} =   await this.supabase.from("chat_messages").select('*').eq("sessionId",this.sessionId).order("createdAt");
       if (data){
